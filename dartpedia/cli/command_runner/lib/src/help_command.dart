...
class HelpCommand extends Command {
  ...
  FutureOr<Object?> run(ArgResults args) async {
    ...
    if (args.hasOption('command')) {
      var (_, commandName) = args.getOption('command');
  ...
  String _renderCommandVerbose(Command cmd) {
    ...
    if (cmd.helpHint != null) {
      buffer.writeln(
        '$indent [Argument] Required? ${cmd.requiresArgument}, Type: ${cmd.helpHint}, Default: ${cmd.defaultValue ?? 'none'}',
      );
    }
